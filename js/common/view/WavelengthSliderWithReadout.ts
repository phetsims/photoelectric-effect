...
export default class WavelengthSliderWithReadout extends Node {
  ...
  public constructor( wavelengthProperty: NumberProperty, providedOptions: WavelengthSliderWithReadoutOptions ) {
    ...
    const sliderTandem = options.tandem.createTandem( NumberControl.SLIDER_TANDEM_NAME );
    ...
    const slider = new HSlider( wavelengthProperty, wavelengthProperty.range, {
      tandem: sliderTandem,
      trackNode: wavelengthTrackNode,
      thumbNode: wavelengthThumbNode
    } );

    const readout = new NumberDisplay( wavelengthProperty, wavelengthProperty.range, combineOptions<NumberDisplayOptions>(
      {},
      DEFAULT_NUMBER_DISPLAY_OPTIONS,
      {
        ...
        tandem: options.tandem.createTandem( 'readout' )
      }
    ) );
